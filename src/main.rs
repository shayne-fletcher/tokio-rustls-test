...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Client { server, size, count, benchmark } => {
...
async fn run_transfers(client: &FileClient, size: u64, count: usize) -> Result<()> {
    println!("Running {} transfers of {} each", count, format_size(size, DECIMAL));
    ...
    if count > 1 {
        ...
        let avg_throughput: f64 = results.iter().map(|r| r.throughput_mbps).sum::<f64>() / count as f64;
